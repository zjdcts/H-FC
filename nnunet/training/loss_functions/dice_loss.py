...
from nnunet.network_architecture.generic_PointRend_Net import point_sample
from torch import nn


class BCEFocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=0.25, reduction='mean', smooth=1e-5):
    ...
    def forward(self, predict, target):
        ...
        pos_map = target
        # neg_map = 1 - target
        pos_map = torch.clamp(pos_map, self.smooth, 1.0 - self.smooth)
        # neg_map = torch.clamp(neg_map, self.smooth, 1.0 - self.smooth)
        pos = pos_map * pt + self.smooth
        logpos = pos.log()
        # neg = neg_map * pt + self.smooth
        # logneg = neg.log()
        # 在原始ce上增加动态权重因子，注意alpha的写法，下面多类时不能这样使用
        # loss = - (1 - self.alpha) * (1 - pt) ** self.gamma * logpos - self.alpha * pt ** self.gamma * logneg
        loss = - (1 - pt) ** self.gamma * logpos
...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    :return:
    """
    # print(net_output.size(), gt.size()) torch.Size([2, 4, 64, 64, 64]) torch.Size([2, 1, 64, 64, 64])
    if axes is None:
